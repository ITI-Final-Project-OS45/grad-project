...
export default function TasksPage() {
  ...
  const currentUserMember = workspaceMembers.find((m) => m.userId._id === currentUserId);
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([TaskService.getTasks(workspaceId), WorkspaceMemberService.getAllMembers(workspaceId)]).then(
      ([tasksResponse, membersResponse]) => {
  ...
  const { handleAddTask, handleMoveTask, handleTaskUpdated, handleTaskRemoved } = useTaskCrudHandlers({
    workspaceId,
    setTasks,
    tasks,
    onError: setError,
  ...
  if (loading)
    return (
      <div className="flex justify-center items-center min-h-screen bg-background">
        <div className="text-lg font-semibold text-foreground">Loading Kanban...</div>
      </div>
    );

  return (
    <main className="flex flex-col items-center min-h-[calc(100vh-64px)] bg-background px-2 sm:px-0">
      {/* Back Button */}
      <div className="flex items-center gap-4 mb-6 pt-4 w-full max-w-6xl">
      ...
      >
        {error && (
          <Alert className="w-full mb-4 border-destructive/30 bg-destructive/5 dark:bg-destructive/10 backdrop-blur-sm">
            <AlertDescription className="flex items-center justify-between">
              <span className="text-destructive dark:text-destructive-foreground">{error}</span>
              <button
        ...
        <Card className="w-full mb-6 rounded-2xl shadow-lg border-border/20 bg-card/98 backdrop-blur-sm p-4 sm:p-8 dark:border-border/10 dark:bg-card/95">
          <CardHeader className="flex flex-col items-center">
            <CardTitle className="text-3xl font-bold text-foreground mb-2 text-center">Task Management Board</CardTitle>
            <p className="text-muted-foreground text-center mb-4">Manage tasks for your workspace efficiently.</p>
